...
import { encodeFunctionData, getAddress, parseAbi, parseEther, type Address, type Hex } from "viem";
...
const deposit = (to: Address = WALLET, id: Hex = REQUEST) => encodeFunctionData({ abi, functionName: "depositNative", args: [to, id] });

describe("bridge allow-list", () => {
  it("accepts the one deposit the quote described: pinned depository, this wallet, this request id, this value", () => {
  ...
  it("refuses another recipient, another request id, another value, another chain or another contract", () => {
    expect(checkTransaction(list, { chainId: 8453, to: RELAY_DEPOSITORY, data: deposit(SOMEONE), value: parseEther("0.1") })).toMatchObject({ ok: false, reason: expect.stringContaining("wallet") });
    expect(checkTransaction(list, { chainId: 8453, to: RELAY_DEPOSITORY, data: deposit(WALLET, OTHER_REQUEST), value: parseEther("0.1") })).toMatchObject({ ok: false, reason: expect.stringContaining("deposit id") });
