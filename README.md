...
Launches run on Robinhood Chain by default and on Base when the command ends with `on base` (added 2026-09-11). Both use o1's own factory for that chain, read fresh from o1's registry by `pnpm o1:sync`; `pnpm abi:vendor` vendors both chains' ABIs. The two differ in one thing: Robinhood mints an ERC-20 through a CREATE2 deployer, so the `01` salt is mined locally from `launchTokenBytecodeHash`, while Base mints through the B20 precompile, so candidates are checked with `getB20Address` in one Multicall3 round trip (`packages/executor/src/salt.ts`, validated against live Base launches). Base tokens live at `0xB20000...01` addresses. The indexer follows both chains (one cursor each: `swaps` and `swaps:base`), the board has a Robinhood / Base switch in the header (Robinhood by default; the choice is kept in a cookie, `?chain=base` in a link wins), token pages, charts, holders and the swap panel follow the pool's chain, and the Universal Router calldata layout is chosen per chain (`routerLayoutFor`). Trades and bridges from a post are still Robinhood only. Set `RPC_BASE` on the indexer and web deployments; the public Base RPCs work but rate-limit.

Arc (chain id 5042, added 2026-09-12) is the third launch chain: `on arc`. Its gas asset is USDC, so the 2 USDC creation fee, dev buys (`devbuy 5` is 5 USDC, capped by `MAX_DEV_BUY_USDC`, at most six decimals because the adapter converts native units by 1e12 and rejects the rest) and shortfall messages are all in USDC; the only registered pair is ERC-20 USDC and a launch there needs no `pair`. Arc runs the same `launchpad-v4-minimal` contracts as Robinhood (ERC-20 token mode, CREATE2 salt, `01` suffix). Sourcify does not index the chain and the explorer o1 links (arc-scan.org) has nothing verified, but arcexplorer.org holds the factory, fee escrow and token deployer as runtime matches, so `pnpm abi:vendor --only arc` vendors them from its API and cross-checks every shared function against the Robinhood contract of the same role; the executor still reads and simulates against the live factory before signing. Explorer links for Arc go to arcexplorer.org. Metadata for Arc launches is pinned through o1's Public API like everywhere else (it lists chain 5042 since 2026-09-12; a failed pin falls back to o1bot's Pinata). Arc launches are refused with a clear reply until `RPC_ARC` is set (`https://rpc.arc-scan.org` is a public endpoint that needs no key; viem itself lists none), and the Privy policy has Arc rules (`pnpm privy:policy`). Since 2026-09-13 Arc is indexed like the other chains (cursor `swaps:arc`, PoolManager `Swap` plus hook `Trade` events, over `RPC_ARC` or the public endpoint) and has its board (USDC pairs only, so no pair chips), token pages (chart, trades, holders through o1's API) and profile rows. Swaps from the site stay closed on Arc because the chain has no Universal Router or Permit2; the token page links to o1 for trading there, and `quoteUsd` prices Arc's USDC at one dollar without the swap addresses the other chains need.
...
| Service | Host | Reads |
| ------- | ---- | ----- |
| web | Vercel, root directory `apps/web` | `DATABASE_URL`, `PRIVY_APP_ID`, `PRIVY_APP_SECRET`, `PRIVY_AUTHORIZATION_PRIVATE_KEY`, `PRIVY_SIGNER_ID`, `NEXT_PUBLIC_PRIVY_APP_ID`, `NEXT_PUBLIC_PRIVY_SIGNER_ID`, `NEXT_PUBLIC_DOCS_URL`, `NEXT_PUBLIC_X_URL`, `NEXT_PUBLIC_GITHUB_URL`, `NEXT_PUBLIC_CONTACT_EMAIL`, `RPC_ROBINHOOD`, `O1_API_URL`, `O1_API_KEY`, `IPFS_GATEWAY`, `LOG_LEVEL`, `LOG_PRETTY`, `FEE_SPLITTER_FACTORY_ROBINHOOD`, `FEE_SPLITTER_FACTORY_BASE`, `FEE_SPLITTER_FACTORY_ARC` |
| bot | Railway | `DRY_RUN`, `SITE_URL`, `DOCS_URL`, `DATABASE_URL`, `QUEUE_DRIVER`, `REDIS_URL`, `PRIVY_APP_ID`, `PRIVY_APP_SECRET`, `PRIVY_AUTHORIZATION_PRIVATE_KEY`, `PRIVY_SIGNER_ID`, `PRIVY_POLICY_ID`, `REFERRER_ADDRESS`, `MAX_TRADE_ETH`, `DEFAULT_USER_TRADE_CAP_ETH`, `TRADE_COOLDOWN_SECONDS`, `MAX_TRADES_PER_USER_PER_DAY`, `TRADE_SLIPPAGE_BPS`, `MAX_BRIDGE_ETH`, `RELAY_API_URL`, `RPC_BASE`, `RPC_ETHEREUM`, `RPC_ARBITRUM`, `RPC_OPTIMISM`, `ALERT_TELEGRAM_BOT_TOKEN`, `ALERT_TELEGRAM_CHAT_ID`, `X_BEARER_TOKEN`, `X_APP_KEY`, `X_APP_SECRET`, `X_APP_ACCESS_TOKEN`, `X_APP_ACCESS_TOKEN_SECRET`, `X_BOT_USER_ID`, `X_BOT_HANDLE`, `X_POLL_MS`, `ANTHROPIC_API_KEY`, `PARSER_MODEL`, `PINATA_JWT`, `O1_API_URL`, `O1_API_KEY`, `RPC_ROBINHOOD`, `RPC_ARC`, `MAX_DEV_BUY_USDC`, `FEE_SPLITTER_FACTORY_ROBINHOOD`, `FEE_SPLITTER_FACTORY_BASE`, `FEE_SPLITTER_FACTORY_ARC`, `MAX_LAUNCHES_PER_USER_PER_DAY`, `LAUNCH_COOLDOWN_SECONDS`, `MAX_DEV_BUY_ETH`, `DEV_BUY_SLIPPAGE_BPS`, `MAX_REPLIES_PER_USER_PER_DAY`, `LOG_LEVEL`, `LOG_PRETTY` |
...
`@o1bot_exchange how many tokens have you launched?`, `total volume?`, `what's trending?`, `how is $CAT doing?`, `what's my ETH balance?`, `my deposit address?`, `how are my launches doing?`, `how much in fees can I claim?`, `did my last buy go through?`, in any language. The parser marks such posts `ask` with a topic (`stats`, `top`, `token`, `wallet`, `launches`, `fees`, `trades`) and the bot reads the figures itself (`apps/bot/src/ask-data.ts`: the `Pool`, `Swap`, `Launch` and `Trade` tables, balances and `FeeEscrow.owed` on chain, holder counts from o1's API), then writes them down as English facts. A second model call phrases the answer in the post's language and the bot's voice (`packages/parser/src/answer.ts`); it may only repeat what the facts say, so a reply carrying any number, address, handle, ticker or link that is not in the facts (or the post) is dropped and the English template in `replies.ts` goes out instead, localized like every other reply. Questions about the poster's own wallet, launches, fees or trades need a linked account and are only ever about the poster; a question about someone else's wallet gets a one-line refusal. Same per-day reply cap as every other reply. Every reply the bot posts (or logs in a dry run) is kept on its `Mention` row as `replyText`, so answers can be reviewed afterwards.

### Token sites (beta)

`@o1bot_exchange launch $CAT "Cash Cat" pair ETH site` (or `site catcoin`) also builds a website for the token at `cat.o1bot.app`; the creator of a token that already exists can post `build a site for $CAT` later. The pipeline checks the name against the slug rules and the reserved labels (`packages/sites/src/slug.ts`), reserves it in `TokenSite` before anything is signed (a taken or impossible name rejects the launch with a suggestion), points the metadata's `website` at it unless the creator gave one, and queues a `TokenSiteJob` once the launch confirms. Reservations without a confirmed launch expire after 30 minutes.

The web app covers the same feature without a post. The launch form (`/launch`) has an "Also build a website" option with an optional name; `POST /api/launch` checks the name and stores it on the request, and the web-launch worker (`apps/bot/src/web-launches.ts`) reserves it right before the launch, points the metadata at it and queues the build exactly as the pipeline does for a post. The profile (`/me`) shows each launch's site (building, live with an edit link, failed with a retry) and a "Build a website" button for a live token without one, which calls `POST /api/site` with the launch id: the web counterpart of `build a site for $CAT`. The board marks tokens with a live site, the token page links it, and `/sites` is a gallery of every live site, each framed from its real address in a sandboxed, scaled-down iframe (the sites' CSP allows the app as a frame ancestor).

The bot worker runs jobs one at a time (`apps/bot/src/site-core.ts`): it briefs the agent from the launch (description and links from the parsed post or the form, the originating post, the creator, the contract address and the read-only token API, the logo with its colour palette) and `packages/sites/src/generate.ts` asks the model, through structured output, for a body fragment and a stylesheet. The first build is published at once and the bot replies under the post with the site and the editor; revisions from the editor are stored as versions the creator publishes.

Sites are data, not deployments, and they live on a sandbox domain of their own, `SITES_ROOT_DOMAIN` (`o1bot.app`), so a site's scripts never share the app's origin. `apps/web/proxy.ts` rewrites `<slug>.o1bot.app` (`*.o1bot.app` must be attached to the Vercel project) to `app/site-render/[slug]`, which renders the published version with the live numbers of the moment and caches it at the edge for a minute. The agent writes HTML, CSS and one inline script; the HTML is sanitized (allow-list of tags and attributes, no forms or inputs, no frames, no event-handler attributes, no `javascript:` URLs, links only to hosts on an allow-list plus the creator's website, https or data images only, a subset of SVG) and served with a Content-Security-Policy, also embedded as a meta element so the editor's preview obeys it: inline scripts only, `connect-src` limited to the app's own API, images from the logo gateways and the app, no frames, forms, objects or media. The o1bot blocks (`<o1bot-stats>`, `<o1bot-buy>`, `<o1bot-address>`, `<o1bot-socials>`, `<o1bot-logo>`, `<o1bot-footer>`) are filled server-side; a script may refresh them from `/api/token/:address` (CORS is open to site origins for that API only). Every footer carries a report link, and `pnpm site:suspend <slug>` takes a site down within a minute (`--restore` brings it back). The creator edits at `/site/<slug>`: instructions on the left become jobs, the preview on the right is the same renderer in a sandboxed frame, any version can be published. `SITES_MODEL` overrides the model.

### Daily recap post

With `RECAP_ENABLED=true` the bot posts once a day from its own account (`apps/bot/src/recap.ts`): launches of the last 24 hours per chain, dollar volume and trade count on o1bot tokens, and the most traded token with its price change, all from the pools and swaps the indexer keeps, followed by the board link. It goes out on the first poll after `RECAP_HOUR_UTC` (default 13, which is 20:00 in Jakarta); the day last handled is kept in the bot cursor table (`recap:last`), so a restart never posts twice and a failed post is retried a minute later. A day with neither a launch nor a trade gets no post. In a dry run the text is logged instead of posted.

### Fee splitter (contracts)

`packages/contracts` (Foundry) holds `FeeSplitterFactory` and `FeeSplitter`: one minimal clone per launch, set as o1's `creatorFeeRecipient`, that pulls the launch's creator fees from o1's `FeeEscrow` and pays them out in the same transaction, a platform share to the o1bot treasury and the rest to the launch's recipients in fixed proportions (`fees to`, splits). o1's escrow keeps fees per recipient address, not per token, which is why every launch needs its own recipient. Clone addresses are deterministic, so the bot can pass them at launch before they exist. Nothing stays in a clone except payouts a recipient could not receive, which that recipient withdraws; the factory owner's `rescue` can never touch those. See `packages/contracts/README.md` for the deployed factories.

In the bot (`apps/bot/src/fee-splitter.ts`, `launch-core.ts`): on a chain with `FEE_SPLITTER_FACTORY_<CHAIN>` set, every launch predicts its clone right after the salt is mined (`factory.predict` with the creator, or the `fees to` account, as sole recipient and the factory's current platform share), stores the address and the configuration on the Launch row, points o1's `creatorFeeRecipient` at the clone in the second transaction (the same one `fees to` always used), then deploys the clone from the gas wallet in `FEE_SPLITTER_GAS_KEY` (`registerWith`, so the announced share holds even if the factory's default changes later; without a gas key the first claim deploys it). A prediction that fails launches without the splitter; the recipient transaction is retried for about 17 seconds while the factory reverts with UnknownLaunchToken (right after a launch the RPC can answer from a node that has not seen the launch block; three launches lost their split this way on 2026-09-13), and one that still fails leaves the fees with the creator, clears the splitter from the row and raises a `fee_split_missed` alert. The success reply states the share ("you keep 80% of the creator fees, o1bot keeps 20% for buyback and burn").

In the web app: `/api/me/overview` reads, per launch with a splitter, what a claim would pay the recipients now (`claimable` on the clone when it is deployed, else `FeeEscrow.owed` for the address it will have, times the recipients' share), and scales "fees earned" by that share; the profile's claim banner lists those launches next to the wallet's own escrow balances, and "Claim $TICKER" sends `claim(currency)` on the clone from the user's wallet (anyone may call it; the clone pays everyone in that transaction). A launch whose clone was never deployed shows "Set up $TICKER" instead, one `registerWith` through the factory with the stored configuration, after which the claim button appears. The wallet's chain list in Privy includes Arc for this. The bot's wallet answer (`ask-data.ts`) adds the recipients' share of what o1 owes the account's splitters to its claimable fees. `apps/web/lib/fee-shares.ts` turns `FEE_SPLITTER_PLATFORM_BPS` into the percentages the landing page, how-it-works and the terms state; keep it equal to the factories' `platformBps`.

### Board as cards

The board has two layouts, switched in the toolbar and remembered per browser with the filter and sort: cards (`apps/web/components/BoardGrid.tsx`, the default: the logo large, ticker, pair and chain, market cap with the day's change and 24-hour volume, the creator and when it launched) and the table with the origin post per row (`BoardTable.tsx`). Both show the same rows in the same order. The cards fill the width in columns of about 200px, two per row on a phone.

### Link previews


Links to the site unfurl on X and elsewhere with images rendered by `next/og` (`apps/web/lib/og.tsx`): `app/opengraph-image.tsx` is the site card (the pitch, the command as a post, the three chains), and `app/token/[address]/opengraph-image.tsx` is one card per token with its logo (fetched and inlined only from the site's own IPFS gateways, without following redirects, since the image URI is metadata written by the launcher; a monogram stands in otherwise or when the gateway is slow), name, ticker, pair, chain, price with 24-hour change, market cap, 24-hour volume, trade count and the creator, cached for five minutes. The layout sets `metadataBase` and `twitter:card = summary_large_image`, so X shows the large card. The renderer takes no variable fonts and no WOFF2, so Bricolage Grotesque Medium and ExtraBold are bundled as TrueType under `apps/web/assets/fonts` (OFL) and shipped with the two routes through `outputFileTracingIncludes`; the chain marks are read from `/public` by URL (`SITE_URL`).
...
`/me` shows the signed-in account: wallet address (copy, explorer, key export), holdings in ETH, USDG and every o1bot token (balances read on chain, valued with the same prices as the board) plus the wallet's USDC on Arc (its gas and only pair, shown under the ERC-20 address; the gas card lists Arc next to Base), withdrawals stay Robinhood-only, the account's launches as creator or as fee recipient with their status, and the creator fees waiting in o1's escrow per paired asset (`FeeEscrow.owed`), plus what each launch's fee splitter can pay out (see "Fee splitter"). Claiming is a transaction the user signs in the browser with their own embedded wallet (`claimFor(wallet, currency)` on the escrow for pre-splitter launches, which always pays the recorded recipient; `claim(currency)` on the launch's splitter otherwise), so the bot's signer is never involved; it needs a little gas on the launch's chain. Gas sponsorship for pregenerated `fees to` recipients is not built yet.
...
4. Create the signer's policy with `pnpm privy:policy`: it registers a separate owner key quorum (private key written to `notes/privy-policy-owner.key`, to be moved offline), a rolling 24-hour value aggregation, and a policy that allows only `createLaunch`, `createLaunchAndBuy` and `setCreatorFeeRecipient` on the active factory (value at most creation fee + `MAX_DEV_BUY_ETH`, daily value under `--daily-cap`; the Arc rules count against a separate rolling 24-hour aggregation in USDC, `--daily-cap-usdc`, default 2000, because value on Arc is native USDC and a shared ETH cap refused every Arc launch above 25 USDC) plus `claimFor`/`claimTo` on the fee escrow; everything else is denied by Privy's enclave. It writes `PRIVY_POLICY_ID` and `NEXT_PUBLIC_PRIVY_POLICY_ID`. Onboarding calls `addSigners` with the key quorum and this policy, which grants o1bot's key signing rights on the user's wallet within those limits. The bot's own allow-list still decides which transactions it will ever sign; a Privy policy can be added on top later.
