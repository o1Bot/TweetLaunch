...
`@o1bot_exchange launch $CAT "Cash Cat" pair ETH site` (or `site catcoin`) also builds a website for the token at `cat.o1bot.exchange`; the creator of a token that already exists can post `build a site for $CAT` later. The pipeline checks the name against the slug rules and the reserved labels (`packages/sites/src/slug.ts`), reserves it in `TokenSite` before anything is signed (a taken or impossible name rejects the launch with a suggestion), points the metadata's `website` at it unless the creator gave one, and queues a `TokenSiteJob` once the launch confirms. Reservations without a confirmed launch expire after 30 minutes.

The bot worker runs jobs one at a time (`apps/bot/src/site-core.ts`): it briefs the agent from the launch (description and links from the parsed post or the form, the originating post, the creator, the logo on the public IPFS gateway with its colour palette) and `packages/sites/src/generate.ts` asks the model, through structured output, for a body fragment and a stylesheet. The first build is published at once and the bot replies under the post with the site and the editor; revisions from the editor are stored as versions the creator publishes.

Sites are data, not deployments. `apps/web/proxy.ts` rewrites `<slug>.o1bot.exchange` (`*.o1bot.exchange` must be attached to the Vercel project; the domain already uses Vercel's nameservers, which wildcards require) to `app/site-render/[slug]`, which renders the published version with the live numbers of the moment and caches it at the edge for a minute. The agent's HTML is sanitized (allow-list of tags and attributes, no event handlers, no `javascript:` URLs, https or data images only, a subset of SVG) and served with a Content-Security-Policy that allows no scripts, no frames and no forms; the o1bot blocks (`<o1bot-stats>`, `<o1bot-buy>`, `<o1bot-address>`, `<o1bot-socials>`, `<o1bot-logo>`, `<o1bot-footer>`) are filled server-side, so nothing on a subdomain can run code and the page still shows live price, market cap, holders and volume. The creator edits at `/site/<slug>`: instructions on the left become jobs, the preview on the right is the same renderer in a sandboxed frame, any version can be published. `SITES_ROOT_DOMAIN` (default `o1bot.exchange`) names the domain; `SITES_MODEL` overrides the model.
