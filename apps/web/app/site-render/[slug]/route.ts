...
import { liveDataFor, lookupSite, SITES_ROOT_DOMAIN } from "@/lib/sites";
...
/**
 * Serves `<slug>.o1bot.exchange`. The proxy rewrites every request on a site
...
function htmlResponse(html: string, status: number, cache: string): Response {
  return new Response(html, {
    status,
    headers: {
      "content-type": "text/html; charset=utf-8",
      "content-security-policy": siteCsp(SITES_ROOT_DOMAIN),
      "cache-control": cache,
...
export async function GET(req: NextRequest, ctx: { params: Promise<{ slug: string }> }) {
  ...
  if (path !== "/" && path !== "/index.html") return htmlResponse(placeholderPage(slug, SITES_ROOT_DOMAIN, "unknown"), 404, "public, s-maxage=60");

  const found = await lookupSite(slug);
  if (found.state !== "live") {
    const status = found.state === "unknown" ? 404 : 200;
    return htmlResponse(placeholderPage(slug, SITES_ROOT_DOMAIN, found.state), status, "public, s-maxage=30, stale-while-revalidate=60");
  }
  const { live, meta } = await liveDataFor(found.site);
  return htmlResponse(renderSite(found.files, meta, live), 200, "public, s-maxage=60, stale-while-revalidate=600");
}
