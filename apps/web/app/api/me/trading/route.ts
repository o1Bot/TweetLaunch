...
/**
 * GET /api/me/trading  → the user's opt-in for buy/sell commands from posts and their per-trade cap.
 * PATCH /api/me/trading { enabled?: boolean; maxTradeEth?: string | null } → update them.
 *
...
type Settings = { enabled: boolean; maxTradeEth: string | null; defaultCapEth: string; maxCapEth: string };
...
async function current(xUserId: string): Promise<Settings> {
  ...
  return { enabled: row?.tradingEnabled ?? false, maxTradeEth: row?.maxTradeWei ? formatEther(BigInt(row.maxTradeWei)) : null, defaultCapEth, maxCapEth };
}
...
export async function PATCH(req: Request) {
  ...
  let body: { enabled?: unknown; maxTradeEth?: unknown };
  try {
    body = (await req.json()) as typeof body;
  } catch {
    return bad("body must be JSON");
  }
  const data: { tradingEnabled?: boolean; maxTradeWei?: string | null } = {};
